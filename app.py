...
with tab_b:
    ...
    if st.session_state.df_working is None:
        ...
        if st.session_state.ai_enabled and st.session_state.openai_key:
            ...
            ai_command = st.text_input("Describe a cleaning operation in plain English",
                                        placeholder='e.g. "replace nulls in price with median and standardize category casing"')
            if st.button("🔍 Suggest Operations") and ai_command:
                with st.spinner("Asking AI..."):
                    try:
                        import openai
                        client = openai.OpenAI(api_key=st.session_state.openai_key)
                        schema_info = {col: str(dtype) for col, dtype in df.dtypes.items()}
                        system_prompt = """You are a data cleaning assistant. Given a dataset schema and a user command, 
return ONLY a valid JSON array of operations. Each operation must have:
...
Return ONLY the JSON array, no markdown, no extra text."""
                        user_prompt = f"Schema: {json.dumps(schema_info)}\nCommand: {ai_command}"
            ...
            if "ai_suggestions" in st.session_state and st.session_state["ai_suggestions"]:
                st.markdown("**Suggested operations — review and confirm:**")
                for i, sug in enumerate(st.session_state["ai_suggestions"]):
                    with st.expander(f"#{i+1} · {sug.get('description', sug['operation'])}", expanded=True):
                        st.json(sug)
                if st.button("✅ Apply All AI Suggestions"):
                    for sug in st.session_state["ai_suggestions"]:
                        push_history()
                        log_step(sug["operation"], sug.get("parameters", {}),
                                 sug.get("affected_columns", []), source="ai_suggested")
                    st.session_state["ai_suggestions"] = []
                    st.success("AI suggestions logged. Note: manual application may be needed for complex ops.")
                    st.rerun()
            st.markdown("---")
                ...
                try:
                    if chart_type == "Histogram" and x_col:
                        ...
                        fig = px.line(line_df, x=x_col, y=y_col, color=c,
                                      title=chart_title or f"{y_col} over {x_col}",
                                      markers=True)
                        st.plotly_chart(fig, use_container_width=True)
                        ...
                        bar_df = bar_df[bar_df[x_col].isin(top_vals)]
                    ...
                    try:
                        ...
                        buf.seek(0)
                        st.download_button("📥 Export Chart as PNG", buf, "chart.png", "image/png", key="chart_png")
                    except Exception:
                        pass
                    plt.close("all")
